...
export class PermissionsService {
  ...
  async getPermissions(query: QueryPermissionsDto) {
    const where: FindOptionsWhere<Permissions> = {
      ...
      ...(query.subject && {
        subject: ILike(`%${this.normalizeString(query.subject)}%`),
      }),
    };
  ...
  async updatePermission(id: string, body: updatePermissionDto) {
    return this.dataSource.transaction(async (manager) => {
      ...
      const subject = body.subject
        ? this.normalizeString(body.subject)
        : permission.subject;
      ...
      if (body.roleIds) {
        ...
        const roleIds = Array.from(
          new Set([...currentRoleIds, ...body.roleIds]),
        );
        ...
        const missingRoleId = body.roleIds.find(
