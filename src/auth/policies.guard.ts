...
import { AppAbility, CHECK_POLICIES_KEY, PolicyHandler } from './decorators/check.policies.decorator';
  ...
  private execPolicyHandler(handler: PolicyHandler, ability: AppAbility){
    ...
    const trackedAbility = new Proxy(ability, {
      get: (target, prop, receiver ) => {
        if(prop === "can"){
          return (checkedAction: Action, checkedSubject: Subjects, ...rest: unknown[]) => {
